...
# --- Task 1: init ---
sandbox
t "init cria dirs"            test -d "$S/.ply/tasks" -a -d "$S/.ply/specs" -a -d "$S/.ply/journal" -a -d "$S/.ply/claims"
t "init cria config"          test -f "$S/.ply/config"
t "init cria template task"   test -f "$S/.ply/tasks/_template.md"
...
t "new incrementa id"   bash -c "cd '$S' && ./ply new 'Outra' && test -f .ply/tasks/002-outra.md"
t "new grava título"    bash -c "cd '$S' && grep -q '^title: Parser XML' .ply/tasks/001-parser-xml.md"
t "new aceita --spec/--dep" bash -c "cd '$S' && ./ply new 'Com deps' --spec 001 --dep '001 002' && grep -q '^depends: 001 002' .ply/tasks/003-com-deps.md && grep -q '^spec: 001' .ply/tasks/003-com-deps.md"
t "spec cria specs/001"  bash -c "cd '$S' && ./ply spec 'Autenticação' && ls .ply/specs/001-*.md"
tf "new --spec sem valor falha limpo" bash -c "cd '$S' && ./ply new 'X' --spec 2>/dev/null"
tf "id corrompido falha com mensagem" bash -c "cd '$S' && printf -- '---\nid: XYZ\ntitle: c\nstatus: todo\n---\n' > .ply/tasks/009-corrompida.md && ./ply new 'Y' 2>/dev/null; r=\$?; rm -f .ply/tasks/009-corrompida.md; exit \$r"
...
bash -c "cd '$S' && ./ply new 'Colide' >/dev/null &&
  awk '{ if (\$0 ~ /^files:/) print \"files: src/x.py\"; else print }' .ply/tasks/003-livre.md > t3 && mv t3 .ply/tasks/003-livre.md &&
  awk '{ if (\$0 ~ /^files:/) print \"files: src/x.py\"; else print }' .ply/tasks/004-colide.md > t4 && mv t4 .ply/tasks/004-colide.md"
...
bash -c "cd '$S' && printf 'TEST_CMD=\"bash\"\n' > .ply/config && ./ply new 'Gate' >/dev/null &&
  awk '{ if (\$0 ~ /^files:/) print \"files: src/ok.sh\"; else if (\$0 ~ /^test:/) print \"test: t/run.sh\"; else print }' .ply/tasks/001-gate.md > t1 && mv t1 .ply/tasks/001-gate.md &&
  ./ply claim 001 --as a1 >/dev/null"
tf "start sem ownership"   bash -c "cd '$S' && ./ply start 001 --as intruso"
t  "start do dono"         bash -c "cd '$S' && ./ply start 001 --as a1 && grep -q '^status: doing' .ply/tasks/001-gate.md"
...
t  "done fecha e libera"   bash -c "cd '$S' && ./ply done 001 --as a1 && grep -q '^status: done' .ply/tasks/001-gate.md && [ ! -d .ply/claims/001 ]"
t  "journal registrou"     bash -c "cd '$S' && grep -q 'done' .ply/journal/001.md && grep -q 'start' .ply/journal/001.md"
...
t  "log do dono"        bash -c "cd '$S' && ./ply log 001 --as a1 'escolhi X porque Y' && grep -q 'escolhi X porque Y' .ply/journal/001.md"
t  "journal imprime"    bash -c "cd '$S' && ./ply journal 001 | grep -q 'escolhi X'"
t  "lesson appenda"     bash -c "cd '$S' && ./ply lesson 'PEM, não PFX' && grep -q 'PEM, não PFX' .ply/tasks/LESSONS.md"
...
tf "fsck pega ciclo"        bash -c "cd '$S' && rm .ply/tasks/003-*.md &&
  awk '{ if (\$0 ~ /^depends:/) print \"depends: 002\"; else print }' .ply/tasks/001-a.md > t && mv t .ply/tasks/001-a.md &&
  ./ply fsck"
...
bash -c "cd '$S' && grep -q '^status: done' .ply/tasks/001-medida.md || { awk '{ if (\$0 ~ /^status:/) print \"status: done\"; else print }' .ply/tasks/001-medida.md > t && mv t .ply/tasks/001-medida.md; echo \"- \$(date -u '+%FT%T') done a1 (gate PASS)\" >> .ply/journal/001.md; }"
bash -c "cd '$S' && awk '{ if (\$0 ~ / done /) print \"- 2100-01-01T00:00:00 done a1 (gate PASS)\"; else print }' .ply/journal/001.md > j && mv j .ply/journal/001.md"
...
t  "init --kit cria skill" bash -c "cd '$S' && ./ply init --kit '$KIT' >/dev/null && test -f .ply/skills/ply-flow/SKILL.md"
t  "init --kit escreve bloco AGENTS" bash -c "cd '$S' && grep -q '<!-- ply:start -->' AGENTS.md && grep -q '<!-- ply:end -->' AGENTS.md && grep -q 'ply — processo' AGENTS.md"
t  "init cria symlink CLAUDE.md" bash -c "cd '$S' && test -L CLAUDE.md && [ \"\$(readlink CLAUDE.md)\" = AGENTS.md ]"
t  "AGENTS.md é arquivo real" bash -c "cd '$S' && test -f AGENTS.md && ! test -L AGENTS.md"
...
t  "init acha \$ROOT/kit" bash -c "cd '$S2' && ./ply init >/dev/null && test -f .ply/skills/ply-flow/SKILL.md"

# Task 11 checks de orçamento do kit (mantém; usa REPO)
t "kit completo"        test -f "$KIT/AGENTS.md" -a -f "$KIT/skills/ply-flow/SKILL.md" -a -f "$KIT/skills/ply-flow/references/ladder.md" -a -f "$KIT/skills/ply-flow/references/decompose.md" -a -f "$KIT/skills/ply-flow/references/dod.md"
t "AGENTS.md ≤1600 bytes" bash -c "[ \$(wc -c < '$KIT/AGENTS.md') -le 1600 ]"
...
t "e2e: loop completo com 2 agentes" bash -c "cd '$S' &&
  ...
  for i in 1 2; do
    awk -v n=\$i '{ if (\$0 ~ /^files:/) print \"files: src/p\" n \".sh\"; else if (\$0 ~ /^test:/) print \"test: t/t\" n \".sh\"; else print }' .ply/tasks/00\$i-*.md > x && mv x .ply/tasks/00\$i-parte-*.md 2>/dev/null || mv x \$(ls .ply/tasks/00\$i-*.md)
  done &&
...
t "install.sh local" bash -c "
  ...
  test -x '$S/ply' &&
  test -f '$S/.ply/skills/ply-flow/SKILL.md' &&
  test -L '$S/CLAUDE.md' &&
  grep -q '<!-- ply:start -->' '$S/AGENTS.md' &&
  test -d '$S/.ply/tasks'
"
...
t "install.sh remoto (tarball local)" bash -c "
  ...
  test -x '$S/ply' && test -f '$S/.ply/skills/ply-flow/SKILL.md' &&
  test -L '$S/CLAUDE.md' &&
  grep -q '<!-- ply:start -->' '$S/AGENTS.md'
"
